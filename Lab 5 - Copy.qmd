...
data1 %>%  ggplot(aes(x=monthly_charges)) +geom_histogram()

data1 %>%  ggplot(aes(x=tenure)) +geom_histogram()
...
data1_train <- rsample::training(data1_split)
data1_test <- rsample::testing(data1_split)

data1_recipe <- data1_train %>% 
  recipes::recipe(formula = churn ~ .) %>% 
  recipes::step_normalize(recipes::all_numeric_predictors()) %>% 
  recipes::step_dummy(recipes::all_nominal_predictors())

data1_recipe %>% recipes::prep(data1_train) %>%  summary()
...
workflow1 <- workflows::workflow() %>% 
  workflows::add_recipe(data1_recipe) %>% 
  workflows::add_model(log_model)

model_fit <- workflow1 %>% 
  parsnip::fit(data1_train)

#step 4
training_aug_result <- broom::augment(model_fit,data1_train)
```
...
training_aug_result %>%  metric(truth = churn, estimate =.pred_class)
```
...
#fitted model put into table
fit1_tbl <- model_fit %>%  broom::tidy() %>% 
  dplyr::arrange(desc(abs(estimate)))

#find tenure for 1 additional year - so filter
fit1_tbl %>% dplyr::filter(term == "tenure") %>% 
  dplyr::pull(estimate) %>% 
  exp()
...
#2 Create WF for knn
workflow1_knn <- workflow1 %>% 
  workflows::update_model(knn_model)
  
#3a Fit knn model to training data
model_fit_knn <- workflow1_knn %>% 
  parsnip::fit(data1_train)

#3b augment data
training_aug_knn_result <- broom::augment(model_fit_knn ,data1_train)
```

```{r}
#Extra test accuracy
training_aug_knn_result %>% metric(truth = churn, estimate =.pred_class)
```

```{r}
#4 
training_aug_knn_result %>% yardstick::roc_auc(.pred_No, truth = churn)

training_aug_knn_result %>% yardstick::roc_curve(.pred_No, truth = churn) %>%  autoplot
```

```{r}
#5

#taken from 3b augment data
test_aug_knn_result <- broom::augment(model_fit_knn ,data1_test)

#taken from 4 
test_aug_knn_result %>% yardstick::roc_auc(.pred_No, truth = churn)

test_aug_knn_result %>% yardstick::roc_curve(.pred_No, truth = churn) %>%  autoplot
...
#2. Fit the CV with 5 in the WF
pred_fit <- workflow1_knn %>% 
  tune::fit_resamples(knn_model_5,
                      control = tune::control_resamples
                      (save_pred = TRUE))

#3. Measure accuracy
pred_fit  %>%  tune::collect_metrics()
```

```{r}
#4. plot curve
pred_fit %>% tune::collect_predictions() %>% 
...
#1b Create WF for knn
workflow1_knn_5 <- workflow1 %>% 
  workflows::update_model(knn_model_tuned)


#2. Create Grid

Grid1 <- dials::grid_regular(dials::neighbors(), levels = 10)

#3. Tuning Results as in Fit the New WF
# Note - resamples mean CV resampling so use CV 

tune_results1 <- tune::tune_grid(
  workflow1_knn_5,
  resamples = knn_model_5,
  grid = Grid1,
...
#Collect metrics
tune_results1  %>%  tune::collect_metrics()
...
```{r}
# show me best model
tune_results1 %>% tune::show_best(metric = "roc_auc")

# select best model
best_model_knn <- tune_results1 %>% tune::select_best(metric = "roc_auc")

#1. Update WF for best model
final_wf_knn <- workflow1_knn_5 %>% 
  tune::finalize_workflow(best_model_knn)

#2. fit the workflow
final_fit <- final_wf_knn %>%  tune::last_fit(data1_split)

#3. collect metrics
final_fit %>% tune::collect_metrics()

#4. get prediction and then plot

final_fit %>% tune::collect_predictions() %>% 
